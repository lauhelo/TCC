...
app.get('/api/usuario/perfil', autenticarToken, async (req, res) => {
    try {
        ...
        res.status(500).json({
            erro: "Erro ao buscar informações do perfil."
        });
    }
});


// 2. ATUALIZAR DADOS DO USUÁRIO AUTOMATICAMENTE
app.put('/api/usuario/perfil', autenticarToken, async (req, res) => {
    try {
        ...
        if (campos.length === 0) {
            return res.status(400).json({
                erro: "Nenhuma informação foi enviada para atualização."
            });
        }

        // Verifica e-mail somente quando ele estiver sendo alterado
        if (Object.prototype.hasOwnProperty.call(dados, "email")) {
            const emailLimpo = String(dados.email || "")
                .trim()
                .toLowerCase();
            ...
            const indiceEmail = valores.findIndex((valor) => {
                return String(valor).trim().toLowerCase() === emailLimpo;
            });

            if (indiceEmail !== -1) {
                valores[indiceEmail] = emailLimpo;
            }
        }
        ...
        res.json({
            mensagem: "Perfil atualizado automaticamente!",
        ...
        res.status(500).json({
            erro: "Erro ao atualizar o perfil no banco de dados."
        });
    }
});
...
// 3. EXCLUIR CONTA
app.delete('/api/usuario/conta', autenticarToken, async (req, res) => {
    try {
        const usuarioId = req.user.id_usuario;
        ...
        res.status(500).json({
            erro: "Erro ao excluir conta do banco de dados."
        });
    }
});
